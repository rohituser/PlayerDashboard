...
def dashboard_examples():
    ...
    if num_valid_svg >= 3:
        ...
        print(f"At least 3 unique SVGs required, only {num_valid_svg} found.")
        points += 12 * num_valid_svg / 3
        
    # check local copies of svg
    with open("dashboard1.svg", "rb") as f:
        assert len(f.read()) > 0
        
    with open("dashboard1-query.svg", "rb") as f:
        assert len(f.read()) > 0
        
    with open("dashboard2.svg", "rb") as f:
        assert len(f.read()) > 0
